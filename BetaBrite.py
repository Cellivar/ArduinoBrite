...
FILE_TYPES = { 'TEXT': "\x41", 'STRING': "\x42", 'DOTS PICTURE': "\x43" }
...
def startPacket( address = ADDRESS, type = 'ALL SIGNS', port = "/dev/ttyS0" ):
...
def startFile( label ):
	...
	if len(branch) == 1 and branch[ len(branch) - 1 ] == "Packet" and lock == 0:
		packet += COMMAND_CODES['WRITE TEXT'] + FILE_LABELS[label]
		branch.append( "File" )
		lock += 1
	else:
		raise PacketLevelException('Method called outside of branch restriction.')
...
def addStringConfig( label, maxSize, kbdProtect = 'LOCKED' ):
	global packet, branch, lock
	...
	if len(branch) == 3 and branch[ len(branch) - 1 ] == "Memory Config" and lock == 2:
		...
		while len(memSize) < 4:
			memSize = "0" + memSize
		packet += FILE_LABELS[label] + FILE_TYPES['STRING'] + KEYBOARD_PROTECT[kbdProtect] + memSize + "0000"
	else:
		raise PacketLevelException('Method called outside of branch restriction.')
